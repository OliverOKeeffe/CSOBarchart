class BarChart {
  ...
  render() {
    ...
    // Calculate the gap between bars in the chart. by subtracting the total width of all bars from the chart width, then dividing by the number of gaps.
    ...
    if (this.chartType === "stacked") {
      maxValue = max(
      this.data.map((d) =>
      his.yValues.reduce((sum, yValue) => sum + d[yValue], 0)
    )
  );
  } else {
